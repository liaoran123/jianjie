...
func createtbs(dbinfo *xbdb.TableInfo) {
	if dbinfo.GetInfo("u").FieldType == nil { //创建用户表
		name := "u"                                                                          //user                                                              //目录表
		fields := []string{"id", "email", "psw", "fahao", "jianjie", "pass", "sj"}           //字段，编码，邮箱，密码，法号。简介，通过（默认。0，则不通过。删除用户设置为0即可）,注册时间。
		fieldType := []string{"int", "string", "string", "string", "string", "bool", "time"} //字段类型
	...
	if dbinfo.GetInfo("wz").FieldType == nil { //创建文章表
		...
		//加入类型名称，是为了空间换时间。
		//pass,1是通过。
		fieldType := []string{"int", "string", "string", "int", "string", "string", "string", "bool", "time"} //字段类型
